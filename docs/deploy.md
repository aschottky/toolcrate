...
2. Render reads `render.yaml` and creates **toolcrate-api** at `https://toolcrate-api.onrender.com`.
...
4. After deploy, open `https://toolcrate-api.onrender.com/api/health` — should return `{"ok":true,...}`.
5. Point Stripe webhooks to `https://toolcrate-api.onrender.com/webhook`.
