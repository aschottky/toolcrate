import"./modulepreload-polyfill-B5Qt9EMX.js";function i(e){return`<li class="pricing-feature">${e}</li>`}function c(e,r=""){return`<div class="pricing-card ${r}">${e}</div>`}function g(){return c(`
    ...
    <ul class="pricing-features">
      ${i("Single page, built around your phone number")}
      ${i("Tap-to-call on every screen")}
      ${i("Your services, hours and service area")}
      ${i("Loads fast on a phone in a truck")}
    </ul>
  ...
  `)}function d(){return c(`
    ...
    <ul class="pricing-features">
      ${i("Everything in the One-Pager")}
      ${i("Up to 10 pages — a page per service")}
      ${i("Photo gallery of your own jobs")}
      ${i("Quote request form straight to your inbox")}
    </ul>
  ...
  `,"pricing-card-featured")}function f(){return c(`
    ...
    <ul class="pricing-features">
      ${i("Online booking or job scheduling")}
      ${i("Multiple locations or crews")}
      ${i("Changes whenever you need them")}
      ${i("You call me directly, not an account manager")}
    </ul>
  ...
  `)}h(document.getElementById("pricing-cards"));(function(){const r=document.getElementById("navCta");if(!r)return;const t=200;let n=!1;function s(){const a=window.scrollY>=t;r.classList.toggle("is-visible",a),n=!1}window.addEventListener("scroll",()=>{n||(n=!0,requestAnimationFrame(s))},{passive:!0}),s()})();const p=new IntersectionObserver(e=>{for(const r of e)r.isIntersecting&&(r.target.classList.add("visible"),p.unobserve(r.target))},{threshold:.1});document.querySelectorAll(".fade-in").forEach(e=>p.observe(e));document.querySelectorAll("[data-counter]").forEach(e=>{const r=parseInt(e.dataset.counter,10),t=e.dataset.suffix||"",n=new IntersectionObserver(([s])=>{if(!s.isIntersecting)return;const a=performance.now(),o=u=>{const l=Math.min((u-a)/2e3,1);e.textContent=`${Math.floor(l*r)}${t}`,l<1&&requestAnimationFrame(o)};requestAnimationFrame(o),n.disconnect()},{threshold:.5});n.observe(e)});document.querySelectorAll(".faq-item").forEach(e=>{const r=e.querySelector(".faq-toggle");r&&r.addEventListener("click",()=>{const t=e.classList.toggle("open");r.setAttribute("aria-expanded",String(t))})});window.innerWidth>=768&&document.querySelectorAll(".conn-line").forEach(e=>{e.style.display="block"});
