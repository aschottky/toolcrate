import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css             */const n=[{id:"starter-tier-outperforms-agencies",title:"The $299 Website That's Outperforming $10,000 Agency Sites",date:"2026-07-20",excerpt:"Discover why high-priced agency websites often fail to convert and how a $299 performance-focused site can drive more revenue for solo-pros and artisans.",category:"Growth",imageUrl:"/blog/starter-tier-1.webp",content:`
    ...
    `},{id:"expensive-free-website-myth",title:'Why Your "Free" Website is Actually Your Most Expensive Expense.',date:"2026-07-16",excerpt:"Discover why $0 website builders are actually a 'stupidity tax' on your business, costing you thousands in lost revenue and mobile friction.",category:"Growth",imageUrl:"/blog/expensive-free-1.webp",content:`
    ...
    `},{id:"website-revenue-leaks-audit",title:"Why Small Business Websites Leak Revenue — The Same Three Failures, Over and Over",date:"2026-07-14",excerpt:"Most small business sites fail the same three tests. Here are the leaks costing you calls — and how to fix them.",category:"UX",imageUrl:"/blog/website-revenue-leaks-1.webp",content:`
      ...
      <p>No cost. No commitment. Just expert-led analysis that shows you what’s really happening with your traffic.</p>
      <p><strong><a href="/try/">Click here to get your Free Expert Preview and stop the leaks today.</a></strong></p>
    `},{id:"decorators-vs-digital-locksmiths",title:"Most Web Designers Are Decorators. I'm a Digital Locksmith. Here's the Difference.",date:"2026-07-13",excerpt:"Stop hiring web decorators who build 'pretty' brochures. Learn how a Digital Locksmith finds revenue leaks and fixes conversion funnels using expert-led UX strategies.",category:"Conversion",imageUrl:"/blog/decorators-1.webp",content:`
    ...
    `}];function p(e){return n.find(t=>t.id===e)??null}function c(){return[...n].sort((e,t)=>e.date<t.date?1:-1)}const i=document.getElementById("blog-root");function l(e){return new Date(`${e}T12:00:00`).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"})}function s(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;")}function u(){document.title="Blog — ToolCrate",document.body.classList.remove("blog-post-view");const e=c(),t=e.map(a=>{const d=a.imageUrl?`
        ...
        `;return`
      <a class="post-card${a.imageUrl?"":" post-card--no-image"}" href="/blog/?post=${encodeURIComponent(a.id)}">
        ${d}
        <div class="post-card__body">
          <div class="post-card__meta">
            <span class="post-card__category">${s(a.category)}</span>
            <span>${s(l(a.date))}</span>
          </div>
          <h2 class="post-card__title">${s(a.title)}</h2>
          <p class="post-card__excerpt">${s(a.excerpt)}</p>
    ...
    ${e.length?`<div class="blog-grid">${t}</div>`:'<p class="blog-empty">No posts yet.</p>'}
  `}function g(e){const t=p(e);if(!t){document.title="Post not found — ToolCrate",document.body.classList.add("blog-post-view"),i.innerHTML=`
      <div class="post-view">
    ...
    `;return}document.title=`${t.title} — ToolCrate`,document.body.classList.add("blog-post-view"),window.scrollTo({top:0,behavior:"auto"}),i.innerHTML=`
    <article class="post-view">
      <a class="post-back" href="/blog/">← Back to Blog</a>
      <div class="post-view__meta">
        <span class="post-view__category">${s(t.category)}</span>
        <span>${s(l(t.date))}</span>
      </div>
      <h1 class="post-view__title">${s(t.title)}</h1>
      <div class="post-prose">${t.content}</div>
  ...
  `}function h(){const e=new URLSearchParams(window.location.search).get("post");e?g(e):u()}h();window.addEventListener("popstate",h);const r=document.getElementById("hamburger"),o=document.getElementById("navLinks");r==null||r.addEventListener("click",()=>{r.classList.toggle("active"),o==null||o.classList.toggle("active")});o==null||o.querySelectorAll("a").forEach(e=>{e.addEventListener("click",()=>{r==null||r.classList.remove("active"),o==null||o.classList.remove("active")})});
